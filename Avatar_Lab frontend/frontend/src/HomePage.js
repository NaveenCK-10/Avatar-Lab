...
export default function HomePage() {
  ...
  useScrollAnimate(heroVisualRef);
  ...
  return (
    <div className={`homepage ${isLightMode ? 'light' : 'dark'}`}>
      ...
      <header ref={heroRef} className={`hero ${heroInView ? 'in-view' : ''}`}>
        ...
        {/* Apply ref to the container div */}
        <div ref={heroVisualRef} className={`hero-visual ${heroInView ? 'in-view' : ''}`}>
          {/* === Use <img> tag for GIF === */}
          {/* Ensure 'Avatar_Video_Generation_Complete.gif' is in the 'public' folder */}
          <img
            // Use a path relative to the public folder (starts with /)
            src="/Avatar_Video_Generation_Complete.gif" // Assuming it's a GIF now
            alt="Animated realistic face illustration"
            className="hero-visual-gif" // Use a class for styling if needed
            width="350" height="350" // Control size
            loading="lazy" // Optional: lazy load
            // Basic error fallback for images
      ...
      <section ref={templatesRef} className={`templates-preview ${templatesInView ? 'in-view' : ''}`}>
        ...
        <div className="template-gallery">
          {[
            ...
            <div key={index} className="template animated-item">
              <div className={`template-media ${template.type === 'audio' ? 'audio-template' : ''}`}>
                {template.type === 'img' ? (
